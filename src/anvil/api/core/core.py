...
def output_development_pack_sizes(config: _AnvilConfig) -> None:
    ...
    click.echo(
        click.style("\r[INFO]: Development pack sizes (uncompressed)", fg="yellow")
    )
    click.echo(
        f"\r  {click.style(f'Behaviour Pack ({behavior_pack_name})', fg='cyan')}: "
        f"{click.style(format_bytes(behavior_pack_size), fg='green')} "
        f"{click.style(f'({behavior_pack_size:,} bytes)', fg='white')}"
    )
    click.echo(
        f"\r  {click.style(f'Resource Pack ({resource_pack_name})', fg='cyan')}: "
...
class _Anvil:
    ...
    _instance = None
    _objects_list: list[AddonObject | object] = []
